...
@Component
@SimpleQrCode
class SimpleQrCodeGenerator : QrCodeGenerator {

    override fun generateQrCode(code: String): ByteArray {
        return try {
            val qrCodeSize = 300
            val link = "https://t.me/HsePermMeetBot?start=" + code
            val hintMap = Hashtable<EncodeHintType, ErrorCorrectionLevel>()
            hintMap[EncodeHintType.ERROR_CORRECTION] = ErrorCorrectionLevel.H

            val qrCodeWriter = QRCodeWriter()
            val bitMatrix = qrCodeWriter.encode(link, BarcodeFormat.QR_CODE, qrCodeSize, qrCodeSize, hintMap)
            ...
            // Добавляем логотип
            addLogoToQr(graphics, image, matrixWidth)
    ...
    private fun addLogoToQr(graphics: Graphics2D, qrImage: BufferedImage, qrSize: Int) {
        try {
